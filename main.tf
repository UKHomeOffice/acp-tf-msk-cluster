...
locals {
  aws_acmpca_certificate_authority_arn = coalesce(
    join("", aws_acmpca_certificate_authority.msk_kafka_with_ca.*.arn),
    join("", aws_acmpca_certificate_authority.msk_kafka_ca_with_config.*.arn),
  )
}
...
resource "aws_iam_user" "msk_acmpca_iam_user" {
  count = var.certificateauthority == "true" && var.acmpca_iam_user_name != "" ? 1 : 0
